...
/**
* 插件 Config schema（0.1.7 设置模型的声明面）。也是浏览器侧 wire 校验依据。
* 全字段 volatile：只投影进设置表单、改值免重挂（官方 ui-theme 同款写法：
* `z.union([...]).default(X).volatile()`）。
*/
const WhaleSettingsSchema = z.object({
	...
	/** 浅色模式图标（favicon.svg）的默认态；无默认：不配置 → 官方原版黑鲸鱼。 */
	black: z.string().pattern(HEX).volatile(),
	/** 深色模式图标（favicon-dark.svg）的默认态；无默认：不配置 → 官方原版白鲸鱼。 */
	blackDark: z.string().pattern(HEX).volatile()
});
...
var src_default = {
	name: "dsh-done-whale",
	/** cordis Plugin.Base.Config：Loader 用它在启动前校验 entry config，
	*  官方 SettingsForms 把它投影成设置表单（volatile 字段才可编辑）。 */
	Config: WhaleSettingsSchema,
	apply(ctx) {
