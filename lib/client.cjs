		...
		*/
		...
		function apply(ctx) {
			...
			*/
			ctx.inject(["sessions", "uiSession"], (sessionCtx) => {
				...
				*  这里按会话 id 关联回列表行，kind 白名单与侧边栏琥珀点同源。 */
				...
				const targetOf = (state, status) => {
					const c = colors();
					for (const row of Object.values(state.byId)) {
						if (row.origin === "subagent") continue;
						if (status.get(row.id)?.completionUnread === true || finishedWhileHidden.has(row.id)) return uri(c.green);
					}
					if (hasPendingInteraction(state.byId, status)) return uri(c.amber);
					return c.black ? uri(c.black) : null;
				};
				sync = () => {
					const state = list.getSnapshot();
					trackEdges(state);
					const next = targetOf(state, statuses.getSnapshot());
			...
			*/
			...
			const update = createUpdateController("0.6.1");
