window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		/**
		* 配置命名空间 = 本插件的 loader entry id（cordis.patch.yml 里 insert 行的 id）。
		* 0.1.7 起官方设置模型按 entry id 寻址：宿主 SettingsForms.describe() 用
		* `entry.options.id` 当 ns，客户端 `ctx.configForms.get(id)` 同键读取。
		* 值与旧版 settings.yaml 的命名空间一致，官方 importLegacyDocument 会把
		* 旧 section 自动迁进同 id entry 的 config（用户配色不丢）。
		*/
		...
		/**
		* 官方图标本身的填充色（设置页输入框的占位/预览值）。
		*
		* 深色模式图标是 DSH 0.1.7-alpha.1 起官方新增的（见 AGENTS.md）。取色方式：
		* 直接读官方 `dsh-web-frontend/dist/` 下两个文件的 `fill`——两图除填充色外逐字
		* 相同，浅色 favicon.svg 是 `#000`、深色 favicon-dark.svg 是 `#fff`。这里写成
		* 6 位形式（`#000000` / `#FFFFFF`），因为 `<input type="color">` 只接受 `#rrggbb`。
		*/
		const DEFAULT_BLACK = "#000000";
		const DEFAULT_BLACK_DARK = "#FFFFFF";
		...
		*   ① 配色：完成/待处理/默认浅色/默认深色四行，每行原生 ColorPicker + hex 文本框 +
		*      「恢复默认颜色」。—— 依赖 configForms 服务（注入面 form，可缺席）；缺席时整块
		*      替换为一行说明，不影响下面的更新块。
		...
		*/
		...
		function WhaleSettingsSection({ form, t, update }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					form === void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						style: MUTED_STYLE,
		...
		function ColorRows({ form, t }) {
			const value = (0, react.useSyncExternalStore)((listener) => form.subscribe(listener), () => form.getSnapshot(), () => form.getSnapshot()).value ?? {};
			...
			const rowsDef = [
				...
				},
				{
					key: "black",
					label: t("blackLabel"),
					fallback: DEFAULT_BLACK
				},
				{
					key: "blackDark",
					label: t("blackDarkLabel"),
					fallback: DEFAULT_BLACK_DARK
				}
			];
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(react_jsx_runtime.Fragment, { children: rowsDef.map(({ key, label, fallback }) => {
				const isDefault = key === "black" || key === "blackDark";
				return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					style: {
						display: "flex",
						flexDirection: "column",
					...
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							children: [
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
									...
									value: current(key, fallback),
									onChange: (e) => write(key, e.target.value),
									placeholder: isDefault ? t("blackHint") : fallback,
									spellCheck: false,
									style: {
										flex: "0 1 33%",
										font: "inherit",
						...
						isDefault && !value[key] && !rows[key]?.draft && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							style: MUTED_STYLE,
		...
		/**
		* 官方只对这几种待处理交互亮琥珀点（dsh-client-ui-workspace 的
		* `visiblePendingKind`，2026-09 核对）。其余 domain 自报的 kind 不进灯——
		* 与侧边栏保持一致，免得"界面没提示、favicon 却黄了"。
		*/
		...
		*/
		...
		function apply(ctx) {
			/** 当前生效的配置色（未配置回官方默认）。 */
			function colors() {
				const value = formRef?.getSnapshot().value ?? {};
				return {
					green: value.green ?? "#22C55E",
					amber: value.amber ?? "#F59E0B",
					black: value.black,
					blackDark: value.blackDark
				};
			}
			/** 颜色配置 form（configForms 服务缺席时保持 undefined）。 */
			let formRef;
			/** locale 可用时的绑定函数（供设置页标签使用）；缺席则回退字面标签。 */
			let localeRef;
			/**
			* favicon 应用面：文档里**全部** `link[rel~="icon"]`。
			*
			* DSH 0.1.7-alpha.1 起前端把单一 icon link 改成两个由 media 二选一的 link
			* （`favicon-dark.svg` 带 `(prefers-color-scheme: dark)`、`favicon.svg` 带
			* `(prefers-color-scheme: light)`），且深色不再由 SVG 内部反白、而是**独立文件**
			* （官方两图除填充色外逐字相同：浅 `#000` / 深 `#fff`）。因此这里**不能**只取
			* 第一个匹配项——只写第一个的话，浅色模式下那个 link 被 media 判为不匹配而失效，
			* 写进去的颜色完全不显示，屏幕上仍是官方原版。
			*
			* 每个 link 按自己的 media 归属模式，默认态各用各的配置色（黑 / 白各一套，
			* 互不影响）；完成色与待处理色是两个模式共用的同一套值。
			*/
			const iconLinks = () => Array.from(document.head.querySelectorAll("link[rel~=\"icon\"]"));
			/** 该 link 属于哪个模式（按 media 判定：不含 dark 即浅色；无 media = 旧版单 link）。 */
			const isDarkLink = (link) => (link.getAttribute("media") ?? "").includes("dark");
			/** 还原目标：应用瞬间逐个记下的原始 href（比硬编码路径更稳：前端改路径也能正确还原）。 */
			const originalHrefs = iconLinks().map((link) => link.href);
			/** 每个 link 当前被写成的状态：颜色 href，或 null = 已还原成官方原值。 */
			let applied = null;
			/** 与“所有 link 都还原成原值”等价的基准态。 */
			const restoredState = () => iconLinks().map(() => null);
			/** 目标态与已应用态是否有差异（applied 为 null 时按“从未写过”处理）。 */
			const differs = (next) => {
				if (applied === null || applied.length !== next.length) return true;
				return next.some((value, index) => value !== applied[index]);
			};
			/** 把目标态写进 DOM（null = 该 link 还原成原值，实现逐 link 独立）。 */
			const applyState = (next) => {
				iconLinks().forEach((link, index) => {
					link.href = next[index] ?? null ?? originalHrefs[index] ?? DEFAULT_HREF;
				});
				applied = next;
			};
			...
			/**
			* ① favicon 状态机：依赖会话列表 + 会话状态（这两个是"灯"的本体）。
			*    服务缺席 = 不亮灯，绝不报错。
			*/
			ctx.inject(["sessions", "uiSession"], (sessionCtx) => {
				const list = sessionCtx.sessions.list;
				const statuses = sessionCtx.uiSession.sessionStatus;
				/** 该会话此刻是否被选中（官方 `mainSessionId()` 同款判定：主视图持有引用）。 */
				const isMainView = (row) => (row?.retainedBy?.mainView ?? 0) > 0;
				/** running true→false 边沿跟踪：完成时恰好被选中且标签页不在台前 →
				*  记入 finishedWhileHidden。官方 completionUnread 只报"未选中时完成"
				*  （observeRunning 里 `!this.isMain(sessionId)` 才 arm），选中的空缺在这里补齐。
				*  台前完成不记（V0-02）；重新运行、会话移除均清除；只跟踪主会话。 */
				const trackEdges = (state) => {
					for (const row of Object.values(state.byId)) {
						...
						if (prev && !row.running) {
							if (isMainView(row) && document.visibilityState === "hidden") finishedWhileHidden.add(row.id);
				...
				/** 系统/浏览器深浅色偏好变化 → 重新应用（0.1.7 的 favicon 由 link 的 media
				*  二选一，偏好切换会换一个生效的 link）。已应用的 href 不变时赋值是幂等的，
				*  这里只为触达浏览器的 favicon 重算。 */
				const themeQuery = window.matchMedia("(prefers-color-scheme: dark)");
				const onThemeChange = () => sync();
				themeQuery.addEventListener("change", onThemeChange);
				/** 主会话中是否存在待处理交互（提问/审批/计划审核）。
				*  0.1.6-alpha.2 起官方把它并入 uiSession.sessionStatus 的
				*  `pendingInteraction`（0.1.5 的独立 pendingInteractions Map 已删）；
				*  这里按会话 id 关联回列表行，kind 白名单与侧边栏琥珀点同源。 */
				const hasPendingInteraction = (byId, status) => {
					for (const row of Object.values(byId)) {
						if (row.origin === "subagent") continue;
						const kind = status.get(row.id)?.pendingInteraction?.kind;
						if (kind !== void 0 && VISIBLE_PENDING_KINDS.has(kind)) return true;
					}
					return false;
				};
				/**
				* 逐个 link 的目标 href；null = 该 link 用官方原值。
				*
				* 状态色的优先级（绿 > 琥珀 > 默认）在两个模式**完全一致**，只有「默认态」
				* 一项按 link 的模式各取各的配置色：浅色 link 用 black、深色 link 用 blackDark。
				* 两者互不影响——只配了浅色时深色那个 link 保持官方白鲸鱼，反之亦然。
				* （无 media 的旧版单 link 按浅色处理，与旧行为一致。）
				*/
				const targetsOf = (state, status) => {
					const c = colors();
					let stateHref = null;
					for (const row of Object.values(state.byId)) {
						if (row.origin === "subagent") continue;
						if (status.get(row.id)?.completionUnread === true || finishedWhileHidden.has(row.id)) {
							stateHref = uri(c.green);
							break;
						}
					}
					if (stateHref === null && hasPendingInteraction(state.byId, status)) stateHref = uri(c.amber);
					return iconLinks().map((link) => {
						if (stateHref !== null) return stateHref;
						const configured = isDarkLink(link) ? c.blackDark : c.black;
						return configured ? uri(configured) : null;
					});
				};
				sync = () => {
					const state = list.getSnapshot();
					trackEdges(state);
					const next = targetsOf(state, statuses.getSnapshot());
					if (next.every((value) => value === null)) {
						restore();
						return;
					}
					if (differs(next)) applyState(next);
				};
			...
			/**
			* ② 颜色配置：依赖 configForms（0.1.7 起替代 settingsScope：按 entry id
			*    取 ConfigForm，读写本插件 Loader 行的 config）。缺席（官方改了服务名）
			*    → 配色块显示"暂时不可用"，状态机沿用默认色，其余功能不受影响。
			*/
			ctx.inject(["configForms"], (formCtx) => {
				const bound = formCtx.configForms.get(SETTINGS_NAMESPACE);
				formRef = bound;
				const unsubscribe = bound.subscribe(() => sync());
				sync();
				formCtx.effect(() => () => {
					unsubscribe();
					if (formRef === bound) formRef = void 0;
				});
			});
			/** ③ 更新控制器：只依赖宿主半的 HTTP API，不依赖任何官方客户端服务。 */
			const update = createUpdateController("1.0.0");
			...
			ctx.inject(["locale"], (localeCtx) => {
				...
				localeCtx.effect(() => localeCtx.locale.register(SETTINGS_NAMESPACE, {
					...
					en: {
						...
						blackLabel: "Default color (light)",
						blackDarkLabel: "Default color (dark)",
